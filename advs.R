...
#diplyr :: rename(type = "Data Type") %>%
#rlang  :: setNames(tolower) %>%
...
vs <- convert_blanks_to_na(vs) #Press F1 between na and brackets to get documentation of function

# see metatools ::combine_supp for combining supplementary SDTM datasets (not covered)
...
# NOTE: Functions appear in a different section of the Enviroment Window
...
# NOTE : Look at the differences between the objects
...
advs <- vs %>%
  ...
  ## Calculate ADTM with imputation (no time but just for fun!- Use for later possibly SMC)
 #derive_vars_dtm(
   #new_vars_prefix = "A",
   #dtc = VSDTC,
...
#Error in derive_vars_dy(., dataset = advs, reference_date = TRTSDT, source_vars = vars(ADT)) :
#unused argument (.)
#unused argument (.)
